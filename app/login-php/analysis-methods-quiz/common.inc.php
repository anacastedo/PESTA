...
function getAllQuizes(string $userId = null){
    ...
    if ($result && $result->num_rows) {
      ...
      if (count($completedQuizIds)) {
          ...
          $result = mysqli_query(
              $DB,
              "SELECT id, current_type, method_type, netlist, image, difficulty, steps FROM quiz WHERE id NOT IN ($completedQuizIdsQuery) ORDER BY RAND() LIMIT 1"
          );
     ...
     $result = mysqli_query($DB, "SELECT id, current_type, method_type, netlist, image, difficulty, steps FROM quiz ORDER BY RAND() LIMIT 1");
...
function getStepsByTheme(string $theme) {
  ...
  $result = mysqli_query($DB, "SELECT id, current_type, method_type, netlist, image, steps FROM quiz ORDER BY RAND()");
