const analysisMethodsQuizAdmin = {
    ...
    registerFormEventListeners(form) {
        ...
        form.querySelector('[data-action="remove-quiz-img"]')?.addEventListener('click', (e) => {
            e.target.parentElement.querySelector('img').remove()
            e.target.parentElement.querySelector('input[name="remove_image"]').value = '1'
            e.target.remove()
        });

        form.querySelector('[data-action="remove-netlist"]')?.addEventListener('click', (e) => {
            e.target.parentElement.querySelector('textarea').remove()
            e.target.parentElement.querySelector('input[name="remove_netlist"]').value = '1'
            e.target.remove()
        });
    },
